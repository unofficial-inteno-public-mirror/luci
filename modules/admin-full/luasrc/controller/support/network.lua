...
module("luci.controller.support.network", package.seeall)

function index()
	local uci = require("luci.model.uci").cursor()
	local page

	page = node("support", "network")
...
--	if page.inreq then
		...
		if has_wifi then
			page = entry({"support", "network", "wireless_join"}, call("wifi_join"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_onoff"}, call("wifi_onoff"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_add"}, call("wifi_add"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_delete"}, call("wifi_delete"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_status"}, call("wifi_status"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_reconnect"}, call("wifi_reconnect"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_shutdown"}, call("wifi_shutdown"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless_scan"}, template("admin_network/wifi_scan"), nil)
			page.leaf = true

			page = entry({"support", "network", "wireless"}, arcombine(template("admin_network/wifi_overview"), cbi("admin_network/wifi")), _("Wireless"), 15)
			page.leaf = true
			page.subindex = true

			--if page.inreq then
				local wdev
				entry({"support", "network", "wireless", "wireless"}, alias("support", "network", "wireless"), "Wireless", 1)
				local net = require "luci.model.network".init(uci)
				for _, wdev in ipairs(net:get_wifidevs()) do
					local wnet
					for _, wnet in ipairs(wdev:get_wifinets()) do
						entry(
							{"support", "network", "wireless", wnet:id()},
							alias("support", "network", "wireless"),
		...
		if nixio.fs.access("/etc/config/dhcp") then
			...
			page = entry({"support", "network", "dhcplease_status"}, call("lease_status"), nil)
			page.leaf = true

			page = node("support", "network", "hosts")
			page.target = cbi("admin_network/hosts")
			page.title  = _("Hostnames")
			page.order  = 40
		end

		page = node("support", "network", "diagnostics")
		page.target = template("admin_network/diagnostics")
		page.title  = _("Diagnostics")
		page.order  = 60

		page = entry({"support", "network", "diag_ping"}, call("diag_ping"), nil)
		page.leaf = true

		page = entry({"support", "network", "diag_nslookup"}, call("diag_nslookup"), nil)
		page.leaf = true

		page = entry({"support", "network", "diag_traceroute"}, call("diag_traceroute"), nil)
		page.leaf = true

		page = entry({"support", "network", "diag_ping6"}, call("diag_ping6"), nil)
		page.leaf = true

		page = entry({"support", "network", "diag_traceroute6"}, call("diag_traceroute6"), nil)
		page.leaf = true
--	end
end
