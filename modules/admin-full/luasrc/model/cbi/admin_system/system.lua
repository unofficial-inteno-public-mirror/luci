...
if guser ~= "user" then
	...
	o = s:taboption("logging", Button, "systemlog", translate("Download logs"), translate("Get all system logs"))
	function o.write()
		local reader = ltn12_popen("system_report 2>/dev/null")
		luci.http.header('Content-Disposition', 'attachment; filename="system_report-%s-%s.log"' % { luci.sys.hostname(), os.date("%Y-%m-%d")})
	...
	local has_videoled = (luci.sys.exec("db get hw.board.lednames"):match("TV"))
